...
LS_URL = os.environ.get('LABEL_STUDIO_BASEURL', 'http://localhost:8080')
...
class YOLOv8Model(LabelStudioMLBase):
    ...
    def predict(self, tasks, **kwargs):
        ...
        try:
            ...
            image_url = task['data']['image']  # Must be a full http(s):// or file:// URL
            ...
            for result in results:
                for i, prediction in enumerate(result.boxes):
                    ...
                    score += prediction.conf.item()
            avg_score = (score / (i + 1)) if predictions else 0   
            return {
                "result": predictions,
                "score": avg_score,
